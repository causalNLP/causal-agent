...
class CodeRunner:
    def __init__(self, safe_exec=True, persistent=False, session_timeout=3600):
        ...
        # Image names (fixed) vs container names (unique per instance)
        self.runner_image = "python-causalscientistrunner"
        self.http_image = "python-causalscientist-http"

        # Unique instance ID, to avoid name collisions
        self.instance_id = uuid.uuid4().hex[:8]
        self.name = f"python-causalscientistrunner-{self.instance_id}"
        self.http_name = f"python-causalscientist-http-{self.instance_id}"
    ...
    def start_persistent_container(self):
        ...
        try:
            print(f"Creating container with image: {self.http_image}")
    ...
    def upload_file(self, local_path, container_path=None):
        ...
        try:
            ...
            response = requests.post(
                ...
                json={"filename": container_path, "content": encoded_content},
                timeout=30,
            )
