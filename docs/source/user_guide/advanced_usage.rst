...
.. code-block:: python
    ...
    def parallel_causal_analysis(analysis_configs: List[Dict[str, Any]], 
                                max_workers: int = 4) -> List[Dict[str, Any]]:
        """Run multiple causal analyses in parallel."""
        
        def run_single_analysis(config):
            start_time = time.time()
            try:
                result = run_causal_analysis(**config)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all analyses
            future_to_config = {
                executor.submit(run_single_analysis, config): config 
    ...
    configs = [
        ...
        {
            'query': 'What is the effect of policy C on indicator W?',
            'dataset_path': 'data/study3.csv'
        }
    ]
    
    results = parallel_causal_analysis(configs, max_workers=2)
...
.. code-block:: python
    ...
    def robust_causal_analysis(query: str, 
                              dataset_path: str,
                              dataset_description: Optional[str] = None,
                              max_retries: int = 3) -> Dict[str, Any]:
        """Run causal analysis with comprehensive error handling."""
        
        for attempt in range(max_retries):
            try:
                logger.info(f"Running causal analysis (attempt {attempt + 1}/{max_retries})")
                ...
                if 'results' not in result:
                    raise ValueError("Invalid result structure: missing 'results' key")
                ...
                if attempt == max_retries - 1:
                    return {'error': str(e)}
                logger.info(f"Retrying analysis...")
        ...
        return {'error': 'Analysis failed after maximum retries'}
...
.. code-block:: python
    ...
    os.environ["CAIS_VERBOSE_LOGGING"] = "true"
    logging.getLogger("causal_agent").setLevel(logging.DEBUG)
    ...
    if 'debug_info' in result:
        ...
        print("Dataset analysis:", debug.get('dataset_analysis'))
        print("Variable identification:", debug.get('variables'))
        print("Method selection reasoning:", debug.get('method_selection'))
