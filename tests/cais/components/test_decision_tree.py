...
from causal_agent.components.decision_tree import (
    ...
    BACKDOOR_ADJUSTMENT, LINEAR_REGRESSION, 
...
def test_no_covariates(base_dataset_analysis, base_variables):
    """Test: No covariates provided -> Linear Regression"""
    variables = base_variables.copy()
    variables["covariates"] = []
    
    # Convert to the format expected by select_method
    properties = {
        ...
        "is_rct": False,
        "has_temporal_structure": base_dataset_analysis.get("temporal_structure", False),
...
def test_rct_no_covariates(base_dataset_analysis, base_variables):
    """Test: RCT, no covariates -> Diff in Means"""
    variables = base_variables.copy()
    ...
    result = select_method(properties)
    # For RCT with no covariates, should select diff_in_means
    from causal_agent.components.decision_tree import DIFF_IN_MEANS
    assert result["selected_method"] == DIFF_IN_MEANS

def test_rct_with_covariates(base_dataset_analysis, base_variables):
    """Test: RCT with covariates -> Linear Regression"""
    variables = base_variables.copy()
